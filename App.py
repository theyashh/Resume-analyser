...
#user interface is made with streamlit in python

def run():
    st.title("Smart Resume Analyser")
    st.sidebar.markdown("# Choose ")
    activities = ["Resume Builder","Resume Analysis", "Admin"] # two different activities are switchable with normal user and admin
    choice = st.sidebar.selectbox("Choose among the given options:", activities)
    link = '[©Developed by Yash Roy](https://www.linkedin.com/in/yash-roy-658956219/)'
    st.sidebar.markdown(link, unsafe_allow_html=True)
    link = '[Check Out My Resume ](https://drive.google.com/file/d/1pWM04M3vW422_sYA3zNbkDhgOP5hlRwM/view?usp=sharing)'
    st.sidebar.markdown(link, unsafe_allow_html=True)
    link = '[Check Out Research Paper ](https://drive.google.com/file/d/1pWM04M3vW422_sYA3zNbkDhgOP5hlRwM/view?usp=sharing)'
    st.sidebar.markdown(link, unsafe_allow_html=True)
    ...
    if choice == 'Resume Builder':
        ...
        def generate_resume(name, email, education, experience, skills):
            pdf = PDF()
            pdf.add_page()
        ...
        def main():
            ...
            st.header("Education")
            education = st.text_area("Enter your education (one per line)").split("\n")

            st.header("Experience")
            experience = st.text_area("Enter your experience (one per line)").split("\n")

            st.header("Skills")
            skills = st.text_area("Enter your skills (one per line)").split("\n")

            if st.button("Generate Resume"):
                resume = generate_resume(name, email, education, experience, skills)
                st.write("Resume Generated!")

                # Save the generated resume as a PDF
                pdf_filename = f"{name}_resume.pdf"
                resume.output(pdf_filename)
                st.write(f"Download your resume [here](/{pdf_filename})")

        if __name__ == "__main__":
            main()

    if choice == 'Resume Analysis':
        st.markdown('''<h4 style='text-align: left; color: #39FF14;'>Upload your resume, and get smart recommendation based on it.</h4>''',
                    unsafe_allow_html=True)
        pdf_file = st.file_uploader("Upload Resume here (PDF format only)", type=["pdf"])
    ...
    if choice == "Admin":
        ...
        if st.button('Login'):
            if ad_user == 'yashroy' and ad_password == 'yash123':
